bott.diff <- function(bott.sign, names_of_groups, ignore_case){
  pairwise.diff <- list()
  n <- 1
  for (i in 1:length(names_of_groups)){
    for (j in 1:length(names_of_groups)){
      ...
      I <- grep(names_of_groups[i],colnames(bott.sign), ignore.case = ignore_case)
      J <- grep(names_of_groups[j],colnames(bott.sign), ignore.case = ignore_case)
