hubs.diff <- function(hubs.sign, names_of_groups, ignore_case){
  pairwise.diff <- list()
  n <- 1
  for (i in 1:length(names_of_groups)){
    for (j in 1:length(names_of_groups)){
      ...
      I <- grep(names_of_groups[i],names(hubs.sign), ignore.case = ignore_case)
      J <- grep(names_of_groups[j],names(hubs.sign), ignore.case = ignore_case)
      pairwise.diff[[n]] <- setdiff(hubs.sign[[I]][,1],hubs.sign[[J]][,1])
