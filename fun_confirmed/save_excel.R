save_excel <- function(all_categories){
  ...
  for (i in names(all_categories$SIG.all)){
    if (nrow(as.data.frame(all_categories$SIG.all[[i]]$tab_summary))){
      ...
      # uno sheet per ogni confronto gpp, con score di similarità, score cor e betw
      ...
      saveWorkbook(summary_pathways, str_c(i,".xlsx"), overwrite = TRUE)
    }
  }
}
